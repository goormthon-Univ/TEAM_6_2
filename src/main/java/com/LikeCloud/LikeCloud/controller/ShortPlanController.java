...
public class ShortPlanController {
    ...
    public ResponseEntity<?> postYearPlan(@RequestHeader("userId") Integer userId, @RequestBody ShortPlanRequestDto shortPlanRequestDto) {
        try {
            // UserRepository를 사용하여 실제로 존재하는 유저를 조회
            User user = userRepository.findById(Long.valueOf(userId))
                    .orElseThrow(() -> new RuntimeException("해당 아이디의 유저를 찾을 수 없습니다."));

            shortPlanService.save(user.getId().intValue(), shortPlanRequestDto);
